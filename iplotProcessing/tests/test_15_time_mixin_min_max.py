...
class MinMaxTimeAlign(unittest.TestCase):
    def setUp(self) -> None:
        ...
        sig1.data_primary = BufferObject(input_arr=np.sin(sig1.time), unit='A')
        
        sig2 = Signal()
        sig2.time = BufferObject(input_arr=[30, 50], unit='s')
        sig2.data_primary = BufferObject(input_arr=np.sin(sig2.time), unit='A')
        
        sig3 = Signal()
        sig3.time = BufferObject(input_arr=[0, 45], unit='s')
        sig3.data_primary = BufferObject(input_arr=np.sin(sig3.time), unit='A')
    ...
    def test_align_1(self):
        time_align(self.signals_set_1, mode=TimeAlignmentMode.MINMAX, kind=InterpolationKind.LINEAR)
    ...
    def test_align_2(self):
        time_align(self.signals_set_2, mode=TimeAlignmentMode.MINMAX, kind=InterpolationKind.LINEAR)
