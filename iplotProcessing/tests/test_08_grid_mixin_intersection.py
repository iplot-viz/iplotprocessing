...
class TestGridIntersection(unittest.TestCase):
    def setUp(self) -> None:
        sig1 = Signal()
        sig1.data_store[0] = BufferObject(
            input_arr=[0, 10, 20, 40, 50], unit='s')
        sig1.data_store[1] = BufferObject(
            input_arr=np.sin(sig1.time), unit='A')

        sig2 = Signal()
        sig2.data_store[0] = BufferObject(input_arr=[30, 50], unit='s')
        sig2.data_store[1] = BufferObject(
            input_arr=np.sin(sig2.time), unit='A')

        sig3 = Signal()
        sig3.data_store[0] = BufferObject(input_arr=[0, 45], unit='s')
        sig3.data_store[1] = BufferObject(
            input_arr=np.sin(sig3.time), unit='A')
    ...
    def test_align_1(self):
        align(self.signals_set_1, mode=GridAlignmentMode.INTERSECTION,
    ...
    def test_align_2(self):
        align(self.signals_set_2, mode=GridAlignmentMode.INTERSECTION,
              kind=InterpolationKind.LINEAR)
